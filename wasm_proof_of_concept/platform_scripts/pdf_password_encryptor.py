...
def main():
    global download_location,not_added
    download_location,not_added=pdf_encryptor('pdf-encryptor',encryptable,passwords)
    not_added=not_convertable.to_py()+not_added
    print(type(download_location),type(not_added))
...
def pdf_encryptor(folder_name,files,passwords):
    ...
    if len(files)==len(passwords):
        ...
        # for file_name,bin_str in files:
            ...
            try:
                ...
                not_added.append(f"{file_name}")
                print(e)
                continue
    else:
        not_added=[file_name for file_name,bin_str in files]
    ...
    if total_files>1:
        ...
        shutil.rmtree(f"{folder_name}")
    print("not_converted: ",not_added)
    return [downloadable_location,not_added]
