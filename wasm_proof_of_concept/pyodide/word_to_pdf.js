...
//   pyodide.globals.set("passwords",[password,])
    ...
    pyodide.runPython(`
        ...
        def main():
            ...
            download_location,not_removed=pdf_decryptor('word-to-pdf',convertable)
        ...
        def pdf_decryptor(folder_name,files):
            empty_root_folder()
            ...
            for file_name,bin_str in files:
                ...
                # bytes_obj=bytes(bin_str,encoding="raw_unicode_escape")
                #try:
                ...
                # convert(bytes_obj.decode(),f"{path_without_extension+fuid}.pdf")
    
                # Write the BytesIO object to a temporary file
                with tempfile.NamedTemporaryFile(delete=False,suffix=".docx") as tmp_file:
                    tmp_file.write(bytes_obj.read())
                
                convert(tmp_file.name,f"{path_without_extension+fuid}.pdf")
                os.remove(tmp_file.name)
    
                # tmp_file.write(word_doc_bytesio.read())
...
async function main() {
    ...
    await micropip.install("docx2pdf");
    // await micropip.install("python-docx",keep_going=true);
    // await micropip.install("pikepdf",keep_going=true);
    // await micropip.install("comtypes");
